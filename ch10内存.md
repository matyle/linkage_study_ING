...
- 大的返回值做法
  ...
  - 编译器会生成一个伪参数，将temp的地址传调用函数，temp地址用eax传出
...
- malloc如何实现？
  ...
  - 程序向系统申请一块堆空间，由程序自己管理这块空间是比较好的。就像是向系统批发一块较大的堆空间，运行库零售给用户
...
- malloc的分配：**对于小于128KB的请求，在现有堆中按堆分配算法为他分配一块空间并返回。大于128KB的请求会使用mmap分配一块匿名空间，然后**在匿名空间中为用户分配空间。
  ```c
...
### 堆分配算法（csapp的虚拟存储器章节 有详细的讲解 记得去复习）
